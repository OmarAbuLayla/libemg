...
def extract_features_from_emg(
    ...
    # Enhanced: robust helper to coerce any statistic into a 1D-per-channel vector.
    def _per_channel(values: np.ndarray | np.ma.MaskedArray, name: str) -> np.ndarray:
        raw = np.ma.getdata(values)
        arr = np.asarray(raw, dtype=float)
        if arr.ndim == 0:
            return np.full(channels, float(arr))
        if arr.ndim > 1:
            try:
                arr = arr.reshape(channels, -1)
            except ValueError as exc:  # pragma: no cover - defensive
                raise ValueError(
                    f"Feature '{name}' produced shape {arr.shape} incompatible with {channels} channels"
                ) from exc
            if arr.shape[1] != 1:
                raise ValueError(
                    f"Feature '{name}' produced {arr.shape[1]} columns; expected a single value per channel"
                )
            arr = arr[:, 0]
        if arr.shape[0] != channels:
            raise ValueError(
                f"Feature '{name}' produced vector of length {arr.shape[0]} but expected {channels}"
            )
    ...
    rms = _per_channel(np.sqrt(np.mean(emg ** 2, axis=1)), "rms")
    mav = _per_channel(np.mean(abs_emg, axis=1), "mav")
    wl = _per_channel(np.sum(np.abs(diff), axis=1), "waveform_length")
    zc = _per_channel(zero_crossings(emg, threshold), "zero_crossings")
    ssc = _per_channel(slope_sign_changes(emg, threshold), "slope_sign_changes")
    wamp = _per_channel(willison_amplitude(emg, wamp_threshold), "willison_amplitude")
    var = _per_channel(np.var(emg, axis=1), "variance")
    std = _per_channel(np.std(emg, axis=1), "std_dev")
    max_abs = _per_channel(np.max(abs_emg, axis=1), "max_abs")
    peak_to_peak = _per_channel(np.ptp(emg, axis=1), "peak_to_peak")
    kurt = _per_channel(
        stats.kurtosis(emg, axis=1, fisher=False, bias=False, nan_policy="omit"), "kurtosis"
    )
    skew = _per_channel(stats.skew(emg, axis=1, bias=False, nan_policy="omit"), "skewness")
    activity, mobility, complexity = hjorth_parameters(emg)
    activity = _per_channel(activity, "hjorth_activity")
    mobility = _per_channel(mobility, "hjorth_mobility")
    complexity = _per_channel(complexity, "hjorth_complexity")
    mean_freq, median_freq, entropy = spectral_descriptors(emg, cfg.sample_rate)
    mean_freq = _per_channel(mean_freq, "mean_frequency")
    median_freq = _per_channel(median_freq, "median_frequency")
    entropy = _per_channel(entropy, "spectral_entropy")

    envelope_mean = _per_channel(np.mean(smoothed_abs, axis=1), "envelope_mean")
    envelope_std = _per_channel(np.std(smoothed_abs, axis=1), "envelope_std")
    envelope_max = _per_channel(np.max(smoothed_abs, axis=1), "envelope_max")
